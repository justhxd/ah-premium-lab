...
- 最低溢价参数：`{min_premium}`（当前 Top30 均值偏移策略不使用该参数）
- 缓存目录：`{cache_dir}`
- 是否强制刷新数据：`{refresh}`
...
A 股年线 = A 股收盘价的 250 个交易日移动平均
H/A 溢价率 = (H 股人民币收盘价 / A 股收盘价 - 1) * 100
```
...
1. 先保留 `A 股收盘价 > A 股 250 日均线` 的 AH 标的；低于、等于或缺少年线的标的不会买入或持有。
2. 将通过年线过滤的 AH 标的按 `H/A 溢价率` 从高到低排序。
3. 取前 30 只计算平均溢价率 `top30_avg`；如果当天不足 30 只，则用当天全部可用标的计算。
4. 取前 10 只作为候选持仓，交易标的是对应 A 股代码（如 `SH600036` / `SZ300750`），不是 H 股代码。
5. 对每只候选标的计算相对前 30 均值的偏移量：
...
7. 如果原始仓位总和小于等于最大总仓位，则保留原始仓位，剩余资金空仓。
8. 如果原始仓位总和大于最大总仓位，则按比例缩放，使总仓位等于最大总仓位。
9. 当天不在目标列表里的 A 股旧持仓会被卖出；如果旧持仓跌到 250 日均线以下，也会因为目标权重变为 0 而触发卖出。
